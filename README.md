...
**Singular and plural both count.** A player cannot know whether today's answer
is `dragon` or `dragons`, and the two are nowhere near each other: across real
puzzles they sit 2,800 ranks apart, `wolf`/`wolves` 30,000, `baby`/`babies`
51,000. Making someone guess which form was picked is a coin toss dressed as a
clue. Both forms are now played as whichever is closer, so typing the singular
when the answer is the plural simply wins. 14,531 pairs, generated by suffix
rules and then checked against the embedding — real pairs score 0.37 to 0.75,
and the misfires (`when`/`whens`, `serf`/`serves`) are all negative. Unlike the
spelling aliases this is not a redirect to a canonical form: both words are
real, so the pairing ships and the client picks per puzzle.
